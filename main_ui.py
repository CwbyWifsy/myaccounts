...
class PasswordManager(QWidget):
    ...
    def _refresh_cards(self):
        ...
        for entry in self.entries:
            ...
            user = QLabel(f"User: {entry['username']}")
            password = QLabel(f"Password: {entry.get('password')}")
            vbox = QVBoxLayout(card)
            vbox.addWidget(name_lbl)
            vbox.addWidget(user)
            vbox.addWidget(password)
            card.mousePressEvent = lambda e, ent=entry: self.show_detail(ent)
            self.card_container.addWidget(card)

    def show_detail(self, entry):
        dlg = DetailDialog(entry, self)
        dlg.exec_()
    ...
    def open_vault(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Open Vault File", "", "Vault Files (*.json);;All Files (*)"
        )
        if not path:
            return
        try:
...
class DetailDialog(QDialog):
    def __init__(self, entry, parent=None):
        super().__init__(parent)
        self.setWindowTitle(entry.get('name', '<Detail>'))
        layout = QFormLayout()
        layout.addRow('Name:', QLabel(entry.get('name', '')))
        layout.addRow('Username:', QLabel(entry.get('username', '')))
        layout.addRow('Password:', QLabel(entry.get('password', '')))
